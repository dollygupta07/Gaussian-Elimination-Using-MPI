...
void distribute_matrix( double *M, double *LM, int x, int y ,int size,int rank)
{
  MPI_Status status;
  int i, j, p;
...
void gather_matrix( double *M, double *LM, int x, int y ,int size,int rank)
{
  MPI_Status status;
  int i, j, p;
...
void parallel_gaussian( double *A, double *b, double *y, int n ,int size,int rank)
{
  MPI_Status status;
  MPI_Request request;
  ...
  distribute_matrix( A, LA, n, n ,size,rank);		// Distribute matrix A and b
  distribute_matrix( b, Lb, 1, n ,size,rank);
...
//    MPI_Bcast( cur_row, n+1, MPI_DOUBLE, k%size, com );
  ...
  gather_matrix( A, LA, n, n ,size,rank);		// Gather matricies
  gather_matrix( y, Ly, 1, n ,size,rank);
} 
...
int main( int argc, char *argv[] )
{
  ...
  else
  {
    ...
    else
    {
       ...
       parallel_gaussian( A, b, y, n,size,rank);
       if(rank==0)
       {
         ftime = MPI_Wtime(); 
         printf("Total TIme is %f\n",ftime-stime);
